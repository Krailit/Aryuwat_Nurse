                        ...
                        foreach (DataColumn dc in returnTable.Columns)
                        {
                            if (dc.ColumnName == rowColumnTitle)
                            {
                                ...
                                if (sumValues)
                                {
                                    try
                                    {
                                        decimal t = drReturn[rowColumnTitle] + "" == "" ? 0 : Convert.ToDecimal(drReturn[rowColumnTitle]);
                                        decimal z = dr[columnZ] + "" == "" ? 0 : Convert.ToDecimal(dr[columnZ]);
                                        drReturn[rowColumnTitle] = t + z;// Convert.ToDecimal(drReturn[rowColumnTitle]) + Convert.ToDecimal(dr[columnZ]);
                                    }
                                    catch
                                    {
