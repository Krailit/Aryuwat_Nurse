    ...
    public partial class FrmSumOfTreatment : DockContent
    {
            ...
            try
            {
                if (ls.SelectedIndex != -1)
                {
                    ...
                    if (value[1].ToUpper() == "CASH")
                    {
                        ...
                        DisplayDefaultPayInComboColumn(LsPayInx, dataGridViewCashTransfer, "PayInCash");
                        //DataGridViewCell cell = dataGridViewCashTransfer.Rows[x].Cells[cname];
                        //if (column.Items.Count > 0)
                        //{
                        //    cell.Value = lsPayin[x];
                        //}
                        ...
                        //dataGridViewCashTransfer["PayInCash", dataGridViewCashTransfer.Rows.Count - 1].Value = "ไทยพานิชย์ 1090";

                        //if (e.ColumnIndex == dataGridViewCashTransfer.Columns.Count - 1)// the combobox column index
                        //{
                        //    if (this.dataGridViewCashTransfer.EditingControl is ComboBox)
                        //    {
                        //        ComboBox cmb = (ComboBox)dataGridViewCashTransfer["PayInCash", dataGridViewCashTransfer.Rows.Count - 1] ;//as ComboBox;
        ...
        private void menuPrintCard_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //c.CN = cn;
                //c.CourseCardID = ccnumber = DermasterSystem.Data.UtilityBackEnd.GenMaxSeqnoValues("CC", BranchID);
                //c.ShowDialog();
