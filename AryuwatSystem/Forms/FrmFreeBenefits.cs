...
{
    public partial class FrmFreeBenefits : DockContent
    {
        ...
        public FrmFreeBenefits()
        {
            InitializeComponent();
            //SetColumns();
           
            FormType = DerUtility.AccessType.Insert;
            this.Closing += new CancelEventHandler(FrmFreeBenefits_Closing);
        }


        void FrmFreeBenefits_Closing(object sender, CancelEventArgs e)
        {
            //Statics.FrmFreeBenefits = null;
        }
        #region Event
        ...
        public void BindApproveComplication(int _pIntseq)
        {
            try
            {
                DataTable dtmb = Entity.Userinfo.MoConfig.Select("[key]='ApproveBen'").CopyToDataTable();
        ...
        private void SelectApplove()
        {
            try
            {
                ...
                //this.Close();
                this.Visible = false;
                //MessageBox.Show(Statics.FrmFreeBenefits.GiftCode);
        ...
        private void FrmFreeBenefits_Load(object sender, EventArgs e)
        {
                  ...
                  if (ISPreview && DerClass.DerUtility.DownLoadImage(saveFullPath, remote_path, filename))
                    ...
                    else //new
                    {
                        ...
                        using (PdfWriter writer = PdfWriter.GetInstance(doc, fs))
                        {
                            ...
                            //BaseFont bfS = BaseFont.CreateFont(Application.StartupPath + "/THSarabunNew Bold.ttf", BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);


                            //BaseFont bfTimes = BaseFont.CreateFont(BaseFont.TIMES_ROMAN, BaseFont.CP1252, false);
                            //iTextSharp.text.Font underline = new iTextSharp.text.Font(bfTimes, 10f, Font.Underline, BaseColor.BLACK);
                            ...
                            //cb.ShowText(this.Text);
                            //============================
                            //Our Phrase will hold all of our chunks
                            
                            //var mainFont = new iTextSharp.text.Font(bfS, 10);
                            ...
                            cb.ShowText(string.Format("จำนวน    {0}     ราคา    {1}",Amount,(Amount*MS_Price).ToString("###,###,###.##")));///"Text at position 30,340."
                                                                                                                 ///
                          

                            cb.SetTextMatrix(30, 240);
                            cb.ShowText(string.Format("หมายเหตุ  {0}", txtNote.Text));///"Text at position 30,340."
