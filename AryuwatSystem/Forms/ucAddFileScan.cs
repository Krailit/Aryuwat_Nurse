...
{
    public partial class ucAddFileScan : UserControl
    {
        private string docFilePath;
        public ucAddFileScan()
        {
            InitializeComponent();
        }
        ...
        private void ucAddFileScan_Load(object sender, EventArgs e)
        {
            try
            {
                SetColumnDgvFile();
            ...
            try
            {
                if (e.ColumnIndex == 4)
                {
                    if (DerUtility.PopMsg(DerUtility.EnuMsgType.MsgTypeConfirm, "ยืนยันการลบไฟล์ \"Confirm delete.\"") == DialogResult.OK)
                    {
                        ...
                        var intStatus = new Business.MedicalOrder().DeleteFileScan(Id, "DELETEFileScan");
      ...
      private void ucAddFileScan_FormClosing(object sender, FormClosingEventArgs e)
      {
          SaveFileScan();
      }
    }
