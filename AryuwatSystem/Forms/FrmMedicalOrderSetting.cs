...
{
    public partial class FrmMedicalOrderSetting : DockContent, IForm
    {

        public FrmMedicalOrderSetting()
        {
            InitializeComponent();
        }

        public FrmMedicalOrderSetting(ref Entity.Customer info)
        {
            InitializeComponent();
        ...
        DataTable dtSU = new DataTable();
        DataTable dtPRO = new DataTable();
        DataTable dtPRA = new DataTable();
        public string VN
        {
            get { return vn; }
            set { vn = value; }
        ...
        public enum TabPageActive
        {
            ...
            tabPromotion = 8,
              ...
              Dictionary<string, Entity.Promotion> dicPromotion = new Dictionary<string, Entity.Promotion>();
              decimal Unpaid = 0;
              decimal NetAmount = 0;
        ...
        private void BindData()
        {
            ...
            txtBalanceRef.Text = dtMed.Rows[0]["PriceTotalRef"] + "" == "" ? "" : Convert.ToDouble(dtMed.Rows[0]["PriceTotalRef"] + "").ToString("###,###,###.##"); ;  
...
//    btnSave.Enabled = false; 
            ...
            //{
                ...
                foreach (DataRow dr in dtSup.Rows)
                {
                        ...
                        double pricePerUnit=0;
                         if (Userinfo.PriceAgency.Contains(dtMed.Rows[0]["CustomerType"] + ""))
                            pricePerUnit= dblCA;
                         else
                             pricePerUnit = dblCL;
        ...
        private void BindPromotionList()
        {
            try
            {
                ...
                info.Tab = "PROMOTION";
                dtPRO = new Business.MedicalSupplies().SelectMedicalSuppliesBySection(info).Tables[0];

                foreach (DataRowView item in dtPRO.DefaultView)
                {
                    object[] myItems = {
                                           false,
                                           item["PRO_Code"] + "",
                ...
                {
                    if (IsExpired(dataRow.Cells["DateStart"].Value + "", dataRow.Cells["DateEnd"].Value + ""))
                    {
        ...
        private void BindDataAesList()
        {
            try
            {
                ...
                //info.MS_Section = "ADI";
                info.Tab = "AESTHETIC";
                dtAE = new Business.MedicalSupplies().SelectMedicalSuppliesBySection(info).Tables[0];


                foreach (DataRowView item in dtAE.DefaultView)
                {
                    object[] myItems = {
                                          false,
                                           item["MS_Section"] + "",
        ...
        private void BindDataSurgeryList()
        {
            try
            {
                ...
                //info.MS_Section = "ODF";
                info.Tab = "SURGERY";
                dtSU = new Business.MedicalSupplies().SelectMedicalSuppliesBySection(info).Tables[0];

                foreach (DataRowView item in dtSU.DefaultView)
                {
                    object[] myItems = {
                                          false,
                                          item["MS_Section"] + "",
        ...
        private void BindDataWellness_antiAgentList()
        {
            try
            {
                ...
                //info.MS_Section = "ODF";
                info.Tab = "WELLNESS";
                dtWE= new Business.MedicalSupplies().SelectMedicalSuppliesBySection(info).Tables[0];

                foreach (DataRowView item in dtWE.DefaultView)
                {
                    object[] myItems = {
                                          false,
                                          item["MS_Section"] + "",
        ...
        private void BindDataPharmacyList()
        {
            try
            {
                ...
                //info.MS_Section = "PMC";
                info.Tab = "PHARMACY";
                dtPRA = new Business.MedicalSupplies().SelectMedicalSuppliesBySection(info).Tables[0];

                foreach (DataRowView item in dtPRA.DefaultView)
                {
                    object[] myItems = {
                                          false,
                                           item["MS_Section"] + "",
        ...
        try
        {
            ...
            if (MoSubType == "PRO")//โปรธรรมดา
            {
                ...
                foreach (DataGridViewRow item in dv.Rows)
                {
                    if ((bool)item.Cells[0].Value == true)
                    {
                        ...
                        dicPromotion.Add(PRO_Code,info);
                    }
                }
            ...
            foreach (DataGridViewRow item in dv.Rows)
            {
                if ((bool)item.Cells[0].Value == true)
                {
                    ...
                    if (PROCredit == "Y") //&& !PROCreditProductGroup.Contains(MS_Section) && (item.Cells["MS_Type"].Value + "").ToUpper() != "S")
                    {
                        if (!PROCreditProductGroup.Contains(MS_Section) || (item.Cells["MS_Type"].Value + "").ToUpper() != "S")
                        {
                            MessageBox.Show("Is not in promotion and cannot select Course Product", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            continue;
                        }
                    }
                    ...
                    object[] myItems = {
                                          ...
                                          false,//Pro
                                           ...
                                           "",
                                           imageList1.Images[0],
                                           dataGridViewSelectList.RowCount
                                       };
                    item.Cells[0].Value = false;
                ...
                if (LsSelectMS_Code.Contains(ms["PRO_Code"].ToString())) continue;
                        ...
                        object[] myItems = {
                                          ...
                                          PROPrice1.ToString("###,###.##"),//PriceTotal
                                           ...
                                           //customerType == "CNT"||customerType == "CNM" ?double.Parse( item.Cells["MS_CLPrice"].Value+"").ToString("###,###.##") :double.Parse( item.Cells["MS_CAPrice"].Value+"").ToString("###,###.##"),//PriceTotal
                                           "",//Other
                                           DateTime.Now.AddYears(1),//ExpireDate
                                           false,//comp
                                           false,//subject
                                           "",//false,//mkt b
                                           "",//false,//gif
    ...
    private void buttonDeleteUp_BtnClick()
    {
        ...
        foreach (DataGridViewRow row in dataGridViewSelectList.Rows)
        {
            ...
            if (MoSubType == "PRO" && PROCredit!="Y")
            {
                ...
                lbPromotion.Text = "";
                dicPromotion.Remove(row.Cells["Code"].Value + "");
            }
            else
            {
                if (Convert.ToBoolean(chk.Value) == true)
        ...
        private void dataGridViewSelectList_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    dblAmount = dataGridViewSelectList.Rows[e.RowIndex].Cells["Amount"].Value + "" == "" ? 0 : decimal.Parse(dataGridViewSelectList.Rows[e.RowIndex].Cells["Amount"].Value + "");
                    dbNumPerC = dataGridViewSelectList.Rows[e.RowIndex].Cells["No./Course"].Value + "" == "" ? 0 : decimal.Parse(dataGridViewSelectList.Rows[e.RowIndex].Cells["No./Course"].Value + "");
                    dbPricePerU = dataGridViewSelectList.Rows[e.RowIndex].Cells["Price/Unit"].Value + "" == "" ? 0 : decimal.Parse(dataGridViewSelectList.Rows[e.RowIndex].Cells["Price/Unit"].Value + "");
                    SPPrice = dataGridViewSelectList.Rows[e.RowIndex].Cells["SpecialPrice"].Value + "" == "" ? 0 : decimal.Parse(dataGridViewSelectList.Rows[e.RowIndex].Cells["SpecialPrice"].Value + "");
                    ...
                    if (PROCredit == "Y" && adddis == false)
                        dataGridViewSelectList.Rows[e.RowIndex].Cells["SpecialPrice"].Value = CalcDiscount(dbPricePerU); //จำนวนทั้งหมด
                    else
                        dataGridViewSelectList.Rows[e.RowIndex].Cells["SpecialPrice"].Value = SPPrice.ToString("###,###,###.##");
        ...
        private void SumPriceMedicalOrder()
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridViewSelectList.Rows)
                {
                    if (row.Visible)
                    {
                        if ((row.Cells["ChkPRO"].Value + "").ToLower() == "true")
                        {
        ...
        {
            Statics.frmMedicalOrderSettingProCredit = new FrmMedicalOrderSettingProCredit();
            //if (cMode == Statics.CallMode.Preview)
            //{
            //    Statics.frmMedicalOrderSettingProCredit.FormType = Utility.AccessType.DisplayOnly;
            //    Statics.frmMedicalOrderSettingProCredit.Text = Text + Statics.StrPreview;
            //}
            ...
            //{
               Statics.frmMedicalOrderSettingProCredit.FormType = DerUtility.AccessType.Insert;
                Statics.frmMedicalOrderSettingProCredit.Text = Text + Statics.StrNewRow;
                Statics.frmMedicalOrderSettingProCredit.SORef = txtSONo.Text;
                Statics.frmMedicalOrderSettingProCredit.ProCreditRemain = ProCreditRemain;
                Statics.frmMedicalOrderSettingProCredit.RefCN = CN;
                Statics.frmMedicalOrderSettingProCredit.RefCN_Name = txtCustomerName.Text;
                Statics.frmMedicalOrderSettingProCredit.ProCreditMoney = ProCreditRemain;
                Statics.frmMedicalOrderSettingProCredit.PRO_Name = PRO_Name;
                Statics.frmMedicalOrderSettingProCredit.PRO_Code = PRO_Code;
                Statics.frmMedicalOrderSettingProCredit.customerType = customerType;
                Statics.frmMedicalOrderSettingProCredit.PROCreditProductGroup = PROCreditProductGroup;
                Statics.frmMedicalOrderSettingProCredit.PROCredit = PROCredit;
            ...
            //Statics.frmMedicalOrderSetting.SO = dgvData.Rows[rowIndex].Cells["SONo"].Value + "";
                ...
                Statics.frmMedicalOrderSettingProCredit.BackColor = Color.FromArgb(255, 230, 217);
                Statics.frmMedicalOrderSettingProCredit.Show(Statics.frmMain.dockPanel1);
        }
            ...
            if (DerUtility.PopMsg(DerUtility.EnuMsgType.MsgTypeConfirm, "ยืนยันการบันทึกข้อมูล \"Confirm Save ?\"") == DialogResult.OK)
                {
                    ...
                    else
                    {
                        if (txtMO.Text.Trim() != "")
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                MessageBox.Show("MO นี้ถูกใช้ไปแล้ว \"MO Is already.\"");
                                return;
                            }
                        }
                        if (txtMO.Text.Trim() == "" && txtSONo.Text.Trim() != "")
                        {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                MessageBox.Show("SO นี้ถูกใช้ไปแล้ว \"SO Is already.\"");
                                return;
                            }
                        }
                    }
            ...
            try
            {
                ...
                List<Entity.MedicalOrderDoc> listMedicalOrderDoc = new List<Entity.MedicalOrderDoc>();
                        ...
                        foreach (DataGridViewRow item in dataGridViewSelectList.Rows)
                        {
                            ...
                            if (consult == ms) info.MedStatus_Code = "2";
        ...
        private void dgvPromotionList_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (IsExpired(dgvPromotionList.Rows[dgvPromotionList.CurrentRow.Index].Cells["DateStart"].Value + "", dgvPromotionList.Rows[dgvPromotionList.CurrentRow.Index].Cells["DateEnd"].Value + ""))
            { ch1.Value = false; }
        }
