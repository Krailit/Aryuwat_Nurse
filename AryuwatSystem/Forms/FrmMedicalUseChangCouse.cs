...
namespace AryuwatSystem.Forms
{
    ...
    public void BindData()
    {
        try
        {
            ...
            DataSet set = new Business.MedicalOrder().SelectMedicalOrderById(VN,this.SORef);
            ...
            {
                SO_Move_Old=this.txtNewSO.Text = table6.Rows[0]["SONew"] + "";
                ...
                if ((table6.Rows[0]["PriceNewBalance"] + "" == "" ? 0 : Convert.ToDecimal(table6.Rows[0]["PriceNewBalance"] + "")) < 0)
                    labelBalance.Text = "ลูกค้าจ่ายเพิ่ม :";
                else labelBalance.Text = "คืนเงินลูกค้า :";
            ...
            foreach (DataRow row in table7.Rows)
            {
                ...
                while (index < rowArray.Length)
                {
                    ...
                    if (strArray.Length > 1)
                    {
                        ...
                        foreach (string str20 in strArray)
                        {
                            ...
                            foreach (DataRow row3 in dataSource.Select(str21))
                            {
                                ...
                                str7 = str7 + row3["NumOfUse"];
                                str19 = row3["ExpireDate"]+"";
                                if (str8 != "")
                                {
                                    str8 = str8 + ":";
                                }
                                num12 = num4 - double.Parse(row3["NumOfUse"] + "");
                                ...
                                num7 = Entity.Userinfo.PriceNormal.Contains(table.Rows[0]["CustomerType"] + "")?num5:num6; //(table.Rows[0]["CustomerType"] + "" == "CNT") || (table.Rows[0]["CustomerType"] + "" == "CNM") ? num5 : num6;
                                if (str9 != "")
                                {
                                    str9 = str9 + ":";
                                }
                               ...
                               IsCancel=row3["IsCancel"]+"".ToUpper()=="Y";
                            }
                        }
                        objArray = new object[] { 
                            false,IsCancel, str2, str3, str4, str5, str6, str7, str8, str9, num.ToString("###,###.##"), str11, str13, str19, str14 == "Y", str15 == "Y", str16 == "Y", 
                            str17 == "Y", str18,MedStatus_NameUse
                         };
                    ...
                    else
                    {
                        ...
                        IsCancel = row2["IsCancel"] + "".ToUpper() == "Y";
                        object[] objArray2 = new object[0x12];
                        objArray2[0] = false;
                        objArray2[1] = IsCancel;
                        objArray2[2] = row2["MS_Code"];
                        objArray2[3] = row2["MS_Name"];
                        objArray2[4] = row2["Amount"];
                        objArray2[5] = row2["MS_Number_C"];
                        objArray2[6] = num4.ToString("###,###.##");
                        num12 = row2["NumOfUse"] + "" == "" ? 0.0 : double.Parse(row2["NumOfUse"] + "");
                        objArray2[7] = num12.ToString("###,##0.##");
                        num12 = num4 - (row2["NumOfUse"] + "" == "" ? 0.0 : double.Parse(row2["NumOfUse"] + ""));
                        objArray2[8] = num12.ToString("###,##0.##");
                        objArray2[9] = num7.ToString("###,###.##");
                        objArray2[10] = double.Parse(row2["PriceAfterDis"] + "") .ToString("###,###.##");// (double.Parse(row2["Amount"]+"") * num7).ToString("###,###.##");
                        objArray2[11] = ((double.Parse(row2["PriceAfterDis"] + "") / num4) * num12).ToString("###,###.##"); //row2["PayByItem"] + "" == "" ? "" : double.Parse(row2["PayByItem"] + "").ToString("###,##0.##");
                        objArray2[12] = row2["ExpireDate"] + "";
                        objArray2[13] = row2["Complimentary"]+"" == "Y";
                        objArray2[14] = row2["MarketingBudget"] + "" == "Y";
                        objArray2[15] = row2["Gift"]+"" == "Y";
                        objArray2[16] = row2["Subject"]+"" == "Y";
                        objArray2[17] = row2["MedicalTab"]+"";
                        //objArray2[17] = row2["MedStatus_NameUse"] + "";
                        objArray = objArray2;
                        this.dataGridViewSelectList.Rows.Add(objArray);
    ...
    private void BindDataUsed(string MS_Code)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(info.VN))
            {
                ...
                {
                    double num = Convert.ToDouble(string.IsNullOrEmpty(view["NumOfUse"] + "") ? "1" : (view["NumOfUse"] + "".Replace(",", "")));
        ...
        {
            DerUtility.PopMsg(DerUtility.EnuMsgType.MsgTypeError, exception.Message);
            DerUtility.MouseOff(this);
        }
    }
    ...
    private void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (this.txtNewSO.Text.Trim() == "" && radioButtonUse.Checked)
            {
                DerUtility.PopMsg(DerUtility.EnuMsgType.MsgTypeInformation, "กรุณาระบุ SO อ้างอิง");
            }
            else
            {
                this.listMS_Code = new List<Data.MedicalOrderUseTrans>();
                this.Info = new Entity.MedicalOrderUseTrans(); 
                this.Info.SOClose = this.lbSO.Text;
                this.Info.SONew = this.txtNewSO.Text.Trim();
                this.Info.SONewOld = SO_Move_Old;
                ...
                //this.Info.ListMsCancel = new List<MedicalOrderUseTrans>();
                foreach (DataGridViewRow row in (IEnumerable) this.dgvUsedTrans.Rows)
                {
                    MedicalOrderUseTrans item = new MedicalOrderUseTrans();
                        ...
                        if (row.Cells["ChkCancel"].Value + "" == "True")
                        {
                            item.IsCancel = "Y";
                        }
                ...
                if ((bool) this.dataGridViewSelectList.Rows[i].Cells[0].Value)
                {
                    ...
                    if (!flag)
                    {
                        ...
                        object[] values = new object[] { 
                               false
                               ,false
                            , this.dataGridViewSelectList.Rows[i].Cells["Code"].Value
                            ...
                            , this.dataGridViewSelectList.Rows[i].Cells["NumOfUse"].Value
    ...
    private void buttonDeleteUp_BtnClick()
    {
        try
        {
            //foreach (DataGridViewRow row in this.dgvUsedTrans.Rows)
            //{
            ...
            //    }
            //}
            for (int i = 0; i < this.dgvUsedTrans.RowCount; i++)
            {
                bool flag = false;
                if ((bool)this.dgvUsedTrans.Rows[i].Cells[0].Value)
                {
                    foreach (DataGridViewRow row in (IEnumerable)this.dgvUsedTrans.Rows)
                    {
                        if (row.Cells["Code"].Value == this.dgvUsedTrans.Rows[i].Cells["Code"].Value)
                        {
                            this.dgvUsedTrans.Rows.Remove(row);
                        }
                    }
    ...
    private void dataGridViewSelectList_CellContentClick(object sender, DataGridViewCellEventArgs e)
    {
        try
        {
            if (e.ColumnIndex == this.dataGridViewSelectList.Columns["ChkMove"].Index )//|| e.ColumnIndex == this.dataGridViewSelectList.Columns["ChkCancel"].Index)
            {
                //this.dataGridViewSelectList.Rows[e.RowIndex].Cells["ChkMove"].Value = false;
                //this.dataGridViewSelectList.Rows[e.RowIndex].Cells["ChkCancel"].Value = false;
    ...
    private void dgvUsedTrans_CellContentClick(object sender, DataGridViewCellEventArgs e)
    {
        try
        {
            if (e.ColumnIndex == this.dgvUsedTrans.Columns["ChkMove"].Index || e.ColumnIndex == this.dataGridViewSelectList.Columns["ChkCancel"].Index)
            {
    ...
    private void FrmMedicalUseChangCouse_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            ToolTip1.SetToolTip(this.btnRemoveRef, "Delete SO Reference");
            ToolTip1.SetToolTip(buttonNewMO, "New SO Reference.");
            this.SetColumnDgvSelectList();
            this.SetColumnsUsed();
            //if (!string.IsNullOrEmpty(this.SORef))
            //{
                this.BindData();
            //}
        }
    ...
    private void NewMedicalOrder()
    {
        FrmMedicalOrderSetting setting = new FrmMedicalOrderSetting {
            ...
            VN = this.txtNewSO.Text.Trim(),
            customerType = this.customerType
        };
        setting.ShowDialog();
        this.txtNewSO.Text = setting.SO;
    ...
    {
        DerUtility.SetPropertyDgv(this.dataGridViewSelectList);
        ...
        DataGridViewCheckBoxColumn dataGridViewCancel = new DataGridViewCheckBoxColumn
        {
            AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells,
            FlatStyle = FlatStyle.Standard,
            ThreeState = false,
            Name = "ChkCancel",
            HeaderText = "ยกเลิกคืนเงิน",
            CellTemplate = new DataGridViewCheckBoxCell(),
            Width=50

        };
        dataGridViewCancel.CellTemplate.Style.BackColor = Color.Beige;

        this.dataGridViewSelectList.Columns.Add(dataGridViewCancel);
       this.dataGridViewSelectList.Columns["ChkCancel"].Visible=false;

        this.dataGridViewSelectList.Columns.Add("Code", "Code");
        //this.dataGridViewSelectList.Columns["Code"].ReadOnly = true;
        this.dataGridViewSelectList.Columns.Add("Name", "รายการ");
        this.dataGridViewSelectList.Columns["Name"].ReadOnly = true;
        this.dataGridViewSelectList.Columns.Add("Amount", "Quantity");
        ...
        this.dataGridViewSelectList.Columns.Add("NumOfUse", "Used");
        this.dataGridViewSelectList.Columns["NumOfUse"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
        this.dataGridViewSelectList.Columns["NumOfUse"].DefaultCellStyle.ForeColor = Color.Blue;
        this.dataGridViewSelectList.Columns["NumOfUse"].ReadOnly = true;
        ...
        //this.dgvUsedTrans.Columns.Add("MedStatus_NameUse", "สถานะการใช้");
    }

    private void SetColumnsUsed()
    {
        DerUtility.SetPropertyDgv(this.dgvUsedTrans);
        ...
        this.dgvUsedTrans.Columns.Add(dataGridViewCancel);
        ...
        this.dgvUsedTrans.Columns.Add("Amount", "Quantity");
    ...
    private void dataGridViewSelectList_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
    {
        try
        {
            if (e.ColumnIndex > -1 && e.RowIndex > -1 && dataGridViewSelectList.Columns[e.ColumnIndex] is DataGridViewCheckBoxColumn)
            {
                ...
                NumOfUse = dataGridViewSelectList["NumOfUse", e.RowIndex].Value + "" == "0" ? 0 : Convert.ToInt16(dataGridViewSelectList["NumOfUse", e.RowIndex].Value + "");
                Amount = dataGridViewSelectList["Amount", e.RowIndex].Value + "" == "0" ? 0 : Convert.ToInt16(dataGridViewSelectList["Amount", e.RowIndex].Value + "");
                NoCourse = dataGridViewSelectList["No./Course", e.RowIndex].Value + "" == "0" ? 0 : Convert.ToInt16(dataGridViewSelectList["No./Course", e.RowIndex].Value + "");
                if (NumOfUse == Amount * NoCourse)
                {
                    ch1.Value = false;
    ...
    private void btnRemoveRef_Click(object sender, EventArgs e)
    {
        try
        {
            if (txtNewSO.Text.Trim() == "") return;
            if (DerUtility.PopMsg(DerUtility.EnuMsgType.MsgTypeConfirmYesNo,Statics.StrConfirmDelete + txtNewSO.Text) == DialogResult.Yes)
            {
                if (new Business.MedicalOrder().DeleteMedicalOrderById("", txtNewSO.Text) == 1)
                {
                    this.Info = new Entity.MedicalOrderUseTrans();
                    this.Info.SOClose = this.lbSO.Text;
                    this.Info.SONew = this.txtNewSO.Text.Trim();
                    this.Info.SONewOld = SO_Move_Old;
                    new Business.MedicalOrder().DeleteMedicalOrderMove(Info);
                    this.txtNewSO.Text = "";
                    this.PriceTotal = (this.txtPriceTotal.Text.Replace(",", "") == "") ? 0M : Convert.ToDecimal(this.txtPriceTotal.Text.Replace(",", ""));
                    this.PriceNewVN = 0;
                    this.PriceNewBalance = this.PriceTotal - this.PriceNewVN;
