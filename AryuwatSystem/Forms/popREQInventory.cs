...
{
    public partial class popREQInventory : DockContent
    {
        ...
        private void AddItemReq(string FixREQNo)
        {
            ...
            try
            {
                ...
                if (Approved == false && Userinfo.EN != EN_Req && !Userinfo.IsAdmin.Contains(Userinfo.EN)) //approve=N  และ ไม่ใช่เข้าของ ไม่ใช่ admin ReadOnly=true  ไม้ต้อง save
                {
                    ReadOnlyx = false;
                }
                ...
                //    {
                //        if (row.Cells["REQNo"].Value + "" == FixREQNo)
                //        {
                //            row.Selected = true;
                //            break;
                //        }
        ...
        private void btnSaveGet_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MedicalSupplies aa = new MedicalSupplies();
                    aa.QueryType = "UPDATE_REQ_STOCK";//INSER_REQ_STOCK
                    aa.MS_Code = item.Cells["_MS_Code"].Value + "";
