...
{
    public partial class popSubItemOther : Form
    {
        ...
        private void popSubItemOther_Load(object sender, EventArgs e)
        {
            try
            {
                BindData();
        ...
        private void SumTotal()
        {
            try
            {
                SUMPriceAfterDis = 0;
                    ...
                    if (SUMPriceAfterDis == 0 && PriceTotal>0)
                    {
                        btnYes.Enabled = false;
                        labelSum.ForeColor = System.Drawing.Color.Red;

                        labelSum.Text = string.Format("ใส่ราคาไม่ถูกต้อง {0}", (SUMPriceAfterDis - PriceTotal).ToString("###,###,###.##")); 
                    }
                    else if (PriceTotal < SUMPriceAfterDis)
                    {
                        btnYes.Enabled = false;
                        labelSum.ForeColor = System.Drawing.Color.Red;

                        labelSum.Text = string.Format("ใส่ราคาเกิน {0}", (SUMPriceAfterDis - PriceTotal).ToString("###,###,###.##"));
                    }
                    else
                    {
                        btnYes.Enabled = true;
