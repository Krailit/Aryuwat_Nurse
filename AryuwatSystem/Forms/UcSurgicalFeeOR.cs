...
{
    public partial class UcSurgicalFeeOR : UserControl
    {
        ...
        {
            //if (DerUtility.PopMsg(DerUtility.EnuMsgType.MsgTypeConfirmYesNo, "ยืนยันการลบข้อมูล", "ลบข้อมูล") == DialogResult.Yes)
            //{
        ...
        private void BindDataUseHistory()
        {
         ...
         public void BindSurgeryFee()
         {
             try
             {
                 ...
                 //cboExMoney.DataSource =DsSurgicalFee.Tables[1];
                ...
                foreach (DataRow dr in dtSurgeryFee.Rows)
                 {
                     ...
                     if (MS_Code == dr["MS_Code"] + "")
                     {
                         ...
                         Quantity = string.IsNullOrEmpty(dr["Amount"] + "") ? 1 : Convert.ToDouble(dr["Amount"] + "");
                          ...
                          if (MS_Type.ToUpper() == "C" && TypeCashier != "WELLNESS")
                              {
                                  ...
                                  if (TypeCashier == "WELLNESS" && MS_Type.ToUpper() == "S")
                                      SPdouble = SPdouble*1;//(SPdouble * Quantity);
                                  if (TypeCashier != "WELLNESS" && MS_Type.ToUpper() == "S")
                          ...
                          if (Userinfo.PriceAgency.Contains(CNType) && TypeCashier != "SURGERY" && TypeCashier != "HAIR" && TypeCashier != "WELLNESS")
                              SPdouble = SPdouble-(SPdouble * Userinfo.AGENCY_RATE);

                          //=====================หมอป้าง ENL57100002 ===== หักค่าห้องก่อน 10,000 เฉพาะเดือน ตุลา 2019
                          string Room = "";
                          if (Entity.Userinfo.FIX_DR_ROOM_CODE.Contains(MS_Code)) //Fix Room Rate 
                          {
                              SPdouble = SPdouble - Convert.ToDouble(Entity.Userinfo.FIX_DR_ROOM_RATE);
                              Room = " หักค่าห้องผ่าตัด "+Entity.Userinfo.FIX_DR_ROOM_RATE.ToString("###,###,###.##");
                          }
                          if (Entity.Userinfo.FIX_DR_ROOM_CODE1.Contains(MS_Code))//Fix Room Rate 1
                          {
                              SPdouble = SPdouble - Convert.ToDouble(Entity.Userinfo.FIX_DR_ROOM_RATE1);
                              Room = " หักค่าห้องผ่าตัด " + Entity.Userinfo.FIX_DR_ROOM_RATE1.ToString("###,###,###.##");
                          }
                          //=====================หมอป้าง ENL57100002 ===== หักค่าห้องก่อน 10,000 เฉพาะเดือน ตุลา 2019
                         ...
                         lbCN_Name.Text = CustomerName;// dr["FullNameThai"] + "";
                         lbCN.Text = CN;// dr["CN"] + "";
                         lbVN.Text = dr["VN"] + "";
                         ...
                         if (dr["SurgicalFeeTyp"] + "" != "") //จำนวนที่คิดค่ามือ
                         {
                             ...
                             if (MS_Code.Contains(":"))
                             {
                                 if (!lsMS_Name.Contains(dr["MS_Code"] + ""))
                                 {
                                     ...
                                     txtMS_Name.Text = txtMS_NameText = string.Format("({0}/{1}/NetAmount {2}/Quantity {3}){4}", dr["MS_Code"] + "", MS_Price.ToString("###,###,###"), pAF == 0 ? "0" : pAF.ToString("###,###,###"), Quantity, System.Environment.NewLine + dr["MS_Name"]) + Room;
                                 }
                             }
                             else txtMS_Name.Text = txtMS_NameText = string.Format("({0}/{1}/NetAmount {2}/Quantity {3}){4}", dr["MS_Code"] + "", MS_Price.ToString("###,###,###"), pAF == 0 ? "0" : pAF.ToString("###,###,###"), Quantity, System.Environment.NewLine + dr["MS_Name"])+Room;
                         }
        ...
        public void BindUcSurgicalFee(string Type,int rowZero)
        {
            ...
            try
            {
               ...
               if (DsSurgicalFee.Tables[2].Select(@where).Any())
               {
                ...
                if (dgvData.Rows.Count > 0 && ManualReset==false)
                    SP = dgvData.Rows[0].Cells["Money"].Value.ToString().Replace(",", "");
                else
                    SP = (double.Parse(SP.ToString().Replace(",", "")) - PharamPrice) + "";
               ...
               if (DsSurgicalFee.Tables[1].Select(@where1).Any())
               {
                   ...
                   if (MarketingBudget != "" || Complimentary == "Y" || Subject == "Y" || FreeType!="" )
                     {
                         SP = "0";
                         txtMS_Name.Text = txtMS_NameText + " => Free";//MarketingBudget
                     }
                   if (GiftCode != "") txtMS_Name.Text = txtMS_NameText + " Gift "+GiftCode;

               }
               
                 //select distinct [MarketingBudget] FROM [Dermaster2016Dev].[dbo].[SupplieTrans]<>""
                 //select distinct [Complimentary] FROM [Dermaster2016Dev].[dbo].[SupplieTrans]<>"Y"
                 //select distinct [Subject] FROM [Dermaster2016Dev].[dbo].[SupplieTrans]<>"Y"
                //=====================silicone ==========
                //bool Nosilicone;
                ...
                MedicalStuffs = new List<MedicalStuff>();
                 foreach (DataRowView item in dtStuffCommission.DefaultView)
                 {
                     ...
                     if (hr == "1") 
                         ...
                         CMHR = (COM1 + COM2).ToString();
                     else if (hr == "3") 
                         CMHR = (COM1 + COM2 + COM3).ToString();
                     else if (Convert.ToInt16(hr) > 3) 
                         CMHR =( COM1 + COM2 + COM3 + ((Convert.ToInt16(hr)-3)*COM3)).ToString();
                     //switch (hr)
                     //{
                     ...
                     if (Expression != "" && !Expression.Contains("SUM") && !Expression.Contains("NET"))
                     {
                         ...
                         Expression = Expression.Replace("SN1", SPNET1);
                         //Expression = Expression.Replace("CO_BY_USE", COM != "" ? COM + "*" + AmountOfUse : "0");   

                         if(CNType=="CNT" || CNType=="CNM")
                            Expression = Expression.Replace("C3PERCEN", COM != "" ? COM : "0");
                         else
                            Expression = Expression.Replace("C3PERCEN","0");
                         
                      

                         Expression = Expression.Replace("CMHR", CMHR != "" ? CMHR : "0");
                         Expression = Expression.Replace("HR", HR != "" ? HR : "0"); //ถ้าไม่มี ชั่วโมง ให้ตัดเป็น 0
                         //Expression = Expression.Replace("CMHR", CMHR != "" ? CMHR + "*" + AmountOfUse : "0"); 
                         ...
                         if(TypeCashier == "HAIR")
                         {
                             ...
                             AmountOfUse =Amount = txtHairG.Text == "" ? Amount : Convert.ToDouble(txtHairG.Text);
                         }
                         Expression = Expression.Replace("AMOUNT", AmountOfUse + "");
                     ...
                     if (stuffCount > 0)
                     {
                         ...
                         moneyPerstuff = (Convert.ToDouble(resultobj) / stuffCount).ToString();
                     }
                     ...
                     foreach (DataRow row in dtStuff.Rows)
                     {
                         if (item["Position_ID"] + "" == row["Position_ID"] + "")
                         {
                             ...
                             stuffInfo.Com_Date = dtpProdate.Value;// DateTime.Now;//ทำไมใช้วันปัจุบัน ทำไมไม่ใช้ วันที่ ทำ  แต่เปลี่ยนแล้ว เมื่อ 01-09-2019
                     ...
                     //                     TextDetail,
                     //                     nameStuff,
                     //                     DicformuraSave[item["Position_ID"]+""]==""?"":Math.Round(double.Parse( DicformuraSave[item["Position_ID"]+""])).ToString("###,###,###.##"),
                     //                     "","",
                     //                     item["Position_ID"]+""
                     //                 };
                          ...
                          var myItems = new[]
                                      {
                                          ...
                                          TextDetail,
                                          nameStuff,
                                          resultobj=="0"?"0.00":double.Parse(resultobj + "").ToString("###,###,###.#0"),//String.Format("{0:F2}", double.Parse(resultobj + "")),//;
                                          "","",
                                          item["Position_ID"]+""
                                      };
                          dgvData.Rows.Add(myItems);
...
//                        BindSurgeryFee();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    info.Position_IDSave += string.Format("{0}:{1}|", dgvData["Position_ID", i].Value + "", dgvData["money", i].Value + ""); 
                }
                //==========================


                info.QueryType = "SAVE";
                //var idMax = DerUtilityBackEnd.GenMaxSeqnoValues("SUR");
                info.SUR_ID = SUR_ID;
                info.CN = lbCN.Text;
                info.VN = lbVN.Text;
                info.Sono = this.Sono;
