...
{
    public partial class FrmCheckCourseList : DockContent, IForm
    {
        ...
        #endregion
        public FrmCheckCourseList()
        {
            InitializeComponent();
            SetColumns();
         
            //BindMedicalSupplies(1);
            dpDateStart.Value = DateTime.Now;
        ...
        private void dgvData_RowPostPaint(object sender, DataGridViewRowPostPaintEventArgs e)
        {
            ...
            {
                dgvData.Rows[e.RowIndex].DefaultCellStyle.Font = new Font(this.Font, FontStyle.Regular);
                dgvData.Rows[e.RowIndex].DefaultCellStyle.ForeColor = Color.Black;
                dgvData.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.GreenYellow;
            }

            //if (Convert.ToDecimal(dgvData.Rows[e.RowIndex].Cells["Balance"].Value.ToString()) <= 0 || 
            //    dgvData.Rows[e.RowIndex].Cells["MOStatus"].Value.ToString().ToLower() == "expired" || 
            //    dgvData.Rows[e.RowIndex].Cells["MOStatus"].Value.ToString().ToLower() == "courseclose")
            //{
        ...
        private void SetColumns()
        {
            ...
            dgvData.Columns["Total"].Width = 70;
            dgvData.Columns.Add("Used", "ใช้ไป");
            dgvData.Columns["Used"].Width = 50;
            dgvData.Columns.Add("Balance", "คงเหลือ");
            dgvData.Columns["Balance"].Width = 60;
            ...
            dgvData.Columns.Add("UsedStatus", "UsedStatus");
            dgvData.Columns.Add("MOStatus", "MOStatus");
        ...
        private void BindDataUsed()
        {
            try
            {
                ...
                dSTmp = new Business.MedicalOrderUseTrans().SelectMedicalOrderUseTransByCN_CheckCouse(info);
                ...
                dtTmp = dSTmp.Tables[0];
                    foreach (DataRowView item in dtTmp.DefaultView)
                    {
                        ...
                        if (Convert.ToDecimal(dgvData.Rows[c].Cells["Balance"].Value.ToString()) <= 0 ||
                    dgvData.Rows[c].Cells["MOStatus"].Value.ToString().ToLower() == "expired" ||
                    dgvData.Rows[c].Cells["MOStatus"].Value.ToString().ToLower() == "courseclose" )//&& dgvData.Rows[c].Cells["Renewal"].Value + "" != ""
                        {
                            dgvData.Rows[c].DefaultCellStyle.Font = new Font(this.Font, FontStyle.Strikeout);
                            dgvData.Rows[c].DefaultCellStyle.BackColor = Color.Gray;
                        }

                        else
                        {
                            dgvData.Rows[c].DefaultCellStyle.BackColor = Color.GreenYellow;
                        }

                        if (Convert.ToDecimal(dgvData.Rows[c].Cells["Balance"].Value.ToString()) > 0 &&
                            dgvData.Rows[c].Cells["MOStatus"].Value.ToString().ToLower() == "expired")
                        {
                            dgvData.Rows[c].DefaultCellStyle.BackColor = Color.Yellow;
                        }
        ...
        private void txtFilter_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                //}
                  for (int u = 0; u < dgvData.RowCount; u++)
                    {
                        if (((dgvData.Rows[u].Cells["MS_Name"].Value + "").ToLower() + (dgvData.Rows[u].Cells["FirstUsedRefMO"].Value + "").ToLower() + (dgvData.Rows[u].Cells["SOno"].Value + "") + (dgvData.Rows[u].Cells["VN"].Value + "")).ToLower().Contains(txtFilter.Text)) //|| (dgvData.Rows[u].Cells["CN_USEDFULLNAME"].Value + "").ToLower().Contains(txtFilter.Text))
                                {
                                    dgvData.Rows[u].Visible = true;
                                }
                                else
        ...
        private void dgvData_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                popCheckCoursePro pop = new popCheckCoursePro();
                pop.VN = dgvData.Rows[e.RowIndex].Cells["VN"].Value + "";
                pop.Sono = dgvData.Rows[e.RowIndex].Cells["SOno"].Value + "";
                pop.Proname = dgvData.Rows[e.RowIndex].Cells["MS_Name"].Value + "";
                pop.ProPrice = dgvData.Rows[e.RowIndex].Cells["Total"].Value + "";
                pop.ShowDialog();
        ...
        private void PrintApplove(bool renew)
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dgvData.Rows)
                {
                    ...
                    if ((ch1.Value + "").ToLower() == "true")
                    {
                        ...
                        foreach (DataGridViewCell cell in row.Cells)
                        {
                            ...
                            //item.MS_Code = radioButtonPro.Checked ? row.Cells["MS_Code"].Value + "|PRO_CREDIT":row.Cells["MS_Code"].Value + "" ;
                            item.MS_Code = row.Cells["MS_Code"].Value + "";
                            item.ListOrder = row.Cells["ListOrder"].Value + "";
                            item.EN = Entity.Userinfo.EN;
                            item.RenewStartDate=Convert.ToDateTime(row.Cells["ExpireDate"].Value + "");
                            
                            if (checkBoxStartdate.Checked)
                            {
                                item.RenewStartDate = dpDateStart.Value;
                               // row.Cells["ExpireDate"].Value = dpDateStart.Value.ToString("yyyy/MM/dd");
                            }
                            else dpDateStart.Value = Convert.ToDateTime(row.Cells["ExpireDate"].Value + "");
                        ...
                        iSexpire = Convert.ToDateTime(dmy) < DateTime.Now;
                        if (iSexpire == false)
                        {
                ...
                obj.RenewAddMonth = Convert.ToInt16(comboBoxPeroid.Text);

          
                    obj.RenewStartDate = dpDateStart.Value;
        ...
        private void btnPrintRenew_Click(object sender, EventArgs e)
        {
            try
            {
                PrintApplove(true);
            //comboBoxPeroid
              //  infoSup.RenewAddMonth =
            int RenewAddMonth=Convert.ToInt16(comboBoxPeroid.Text);
                int? intStatus = new Business.MedicalOrder().InsertSupplieTransRenewal(infoSup, RenewAddMonth);
                 BindDataUsed();
            }
            catch (Exception ex)
            {
        ...
        private void resetRenewalToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SupplieTrans item = new SupplieTrans();
                    ...
                    item.ListOrder = row.Cells["ListOrder"].Value + "";
                    item.EN = Entity.Userinfo.EN;
                    item.ExpireDate = DateTime.Now.ToString();
                    item.RenewStartDate = DateTime.Now;
        ...
        private void radioButtonNormal_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataRowView item in dtTmp.DefaultView)
                {
                    ...
                    if (Convert.ToDecimal(dgvData.Rows[c].Cells["Balance"].Value.ToString()) <= 0 ||
                       dgvData.Rows[c].Cells["MOStatus"].Value.ToString().ToLower() == "expired" ||
                       dgvData.Rows[c].Cells["MOStatus"].Value.ToString().ToLower() == "courseclose")
                    {
                        dgvData.Rows[c].DefaultCellStyle.Font = new Font(this.Font, FontStyle.Strikeout);
                        dgvData.Rows[c].DefaultCellStyle.BackColor = Color.Gray;
                    }
        ...
        private void SetStatusColor()
        {
            try
            {
                ...
                {
                    if (Convert.ToDecimal(dataRow.Cells["Balance"].Value.ToString()) <= 0 ||
                        dataRow.Cells["MOStatus"].Value.ToString().ToLower() == "expired" ||
                        dataRow.Cells["MOStatus"].Value.ToString().ToLower() == "courseclose")
                    {
        ...
        private void btnGV_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dgvData.Rows)
                {
                    ...
                    {
                        fm.CN = CN;
                        fm.CustomerName = CustName;
                            fm.SOno= row.Cells["SONo"].Value + "";
                            fm.VN = row.Cells["VN"].Value + "";
                            fm.MS_Code = row.Cells["MS_Code"].Value + "";
                            fm.ListOrder = row.Cells["ListOrder"].Value + "";
                            break;
                    }
                }
        ...
        private void checkBoxStartdate_Click(object sender, EventArgs e)
        {
            dpDateStart.Enabled = checkBoxStartdate.Checked;
        }
    }
}
