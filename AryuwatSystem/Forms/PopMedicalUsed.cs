...
namespace AryuwatSystem.Forms
{
    public partial class PopMedicalUsed : Form
    {
        ...
        public decimal AmountBalanceDecimal { get; set; }//คงเหลือ
        ...
        private string GetCOnumber()
        {
            ...
            try
            {
                ...
                int yy = dateTimePickerCreate.Value.Year;//2020
               
                if (yy < 2500) yy += 543;
                yy -= 2500;
                int mm = dateTimePickerCreate.Value.Month ;
                
                string ss = string.Format("{0}|{1}|{2}", "CO-",yy+"",mm.ToString("00"));
                co = CO = AryuwatSystem.Data.UtilityBackEnd.GenMaxSeqnoValues(ss);
            }
            catch (Exception ex)
            {
        ...
        private void PopMedicalUsed_Load(object sender, EventArgs e)
        {
            ...
            txtCoursID1.Text = CN;
            txtCoursID2.Text = CourseCardID;

            txtUsedCN.Text = CN;
            txtUsedName.Text = CustomerName;
            ...
            if (!Userinfo.IsAdmin.Contains(Userinfo.EN) && !Userinfo.IS_ADMIN_EDIT.Contains(Userinfo.EN)  && !Userinfo.IS_ADMIN_JOBCOST.Contains(Userinfo.EN) && dateTimePickerCreate.Value.Date < DateTime.Now.Date)
            {
                ...
                picBalManual.Visible = false;
                
            }
            else
            {
                picBalManual.Visible = true;
            }
            //SumAmountUse();
        }
        ...
        private void SetColumns()
        {
            ...
            dgvUsedTrans.Columns["DateOfUse"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
              ...
              dgvUsedTrans.Columns.Add("AmountBalance", "AmountBalance");
              dgvUsedTrans.Columns.Add("Shot", "Shot");
              dgvUsedTrans.Columns.Add("GiftCode", "GiftCode");
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(info.VN))
                {
                    ...
                    foreach (DataRowView item in dtTmp.DefaultView)
                    {
                        ...
                        LastUsed = DateTime.Parse(item["DateOfUse"] + "") > LastUsed ? DateTime.Parse(item["DateOfUse"] + "") : LastUsed;
                    }
        ...
        private void SaveData()
        {
            try 
	        {
                ...
                if (statusSave == "INSERT")
                {
                    ...
                    if (decimal.Parse(txtUseNew.Text.Trim()) > (lblBalance.Text==""?0:decimal.Parse(lblBalance.Text.Trim())))
                    {
                ...
                useInfo.GiftCode = txtGiftCode.Text;

                //useInfo.AmountBalance = Convert.ToDecimal(AmountBalance.Replace(",", "")) == 0 ? 0 : Convert.ToDecimal(AmountBalance.Replace(",", "")) - (txtUseNew.ReadOnly ? 0 : decimal.Parse(txtUseNew.Text.Trim()));
                AmountBalance = AmountBalance == "" ? "0" : AmountBalance;
                useInfo.AmountBalance  = AmountusedClick == txtUseNew.Text.Trim() ? Convert.ToDecimal(AmountBalance.Replace(",", "")) : (Convert.ToDecimal(AmountBalance) + (AmountusedClick == "" ? 0 : Convert.ToDecimal(AmountusedClick))) - useInfo.AmountOfUse;//AmountBalance บวกคืน AmountusedClick หัก AmountOfUse
                
                if (textboxFormatDoubleUseBal.Visible)
                {
                    useInfo.AmountBalance = textboxFormatDoubleUseBal.Text.Trim()==""?0:decimal.Parse(textboxFormatDoubleUseBal.Text.Trim());
                    useInfo.AmountBalance = useInfo.AmountBalance < 0 ? 0 : useInfo.AmountBalance;

                }
                this.AmountBalanceDecimal = useInfo.AmountBalance;
                useInfo.DateOfUse = dateTimePickerCreate.Value;
                useInfo.CN_USED = txtUsedCN.Text == "" ? CN : txtUsedCN.Text;
                useInfo.Shot = txtShot.Text == "" ? 0 : Convert.ToDecimal(txtShot.Text);
                if (statusSave == "INSERT")
                    lbCO.Text = GetCOnumber();
                ...
                useInfo.CourseCardID = txtCoursID2.Text.Trim();
                useInfo.Shot = txtShot.Text == "" ? 0 : Convert.ToDecimal(txtShot.Text); ;
                
                UsedName = txtUsedName.Text;
                    CNUsed = txtUsedCN.Text;
                    CurrentUsed = txtUseNew.Text.Trim();
        ...
        public void BindDataUsed(int rindex)
        {
            try
            {
                string id = "";
                if (MutID == "" || rindex >= 0)
                {
                    id =MutID= dgvUsedTrans.Rows[rindex].Cells["Id"].Value + "";
                }
                else
                {
                    foreach (DataGridViewRow row in dgvUsedTrans.Rows)
                    {
                        if (row.Cells["Id"].Value + "" == MutID)
                        {
                            rindex = row.Index;
                            break;
                        }
                ...
                txtUseNew.Text = AmountusedClick= dgvUsedTrans.Rows[rindex].Cells["Amount"].Value + "";
                AmountBalance = dgvUsedTrans.Rows[rindex].Cells["AmountBalance"].Value + "" == "" ? AmountTotal : dgvUsedTrans.Rows[rindex].Cells["AmountBalance"].Value + "";//(Convert.ToDecimal(AmountBalance.Replace(",", "")) + decimal.Parse(txtUseNew.Text.Trim())).ToString("###,###,###.##");
                Entity.MedicalStuff info = new MedicalStuff();
                info.VN = VN;
                info.MS_Code = MS_Code;
                info.ListOrder = dgvUsedTrans.Rows[rindex].Cells["ListOrder"].Value + "";

                info.UseTransId = dgvUsedTrans.Rows[rindex].Cells["Id"].Value + "";
                lbCO.Text = dgvUsedTrans.Rows[rindex].Cells["CO"].Value + "";
                txtUsedCN.Text = dgvUsedTrans.Rows[rindex].Cells["CN_USED"].Value + "";
                txtUsedName.Text = dgvUsedTrans.Rows[rindex].Cells["CN_USEDFULLNAME"].Value + "";
                txtRemark.Text = dgvUsedTrans.Rows[rindex].Cells["Remark"].Value + "";
                checkBoxSwap.Checked = dgvUsedTrans.Rows[rindex].Cells["swap"].Value + "" == "Y";
                EN_REQ = dgvUsedTrans.Rows[rindex].Cells["EN_REQ"].Value + "";
                lbREQ.Text = dgvUsedTrans.Rows[rindex].Cells["Name_REQ"].Value + "";
                textboxFormatDoubleUseBal.Text = dgvUsedTrans.Rows[rindex].Cells["AmountBalance"].Value + "" == "" ? Convert.ToDecimal(AmountTotal).ToString("###,###,###.##") : Convert.ToDecimal(dgvUsedTrans.Rows[rindex].Cells["AmountBalance"].Value + "").ToString("###,###,###.##");
                txtShot.Text = dgvUsedTrans.Rows[rindex].Cells["Shot"].Value + "" == "" ? "0" : Convert.ToDecimal(dgvUsedTrans.Rows[rindex].Cells["Shot"].Value + "").ToString("###,###,###.##");
                txtGiftCode.Text = dgvUsedTrans.Rows[rindex].Cells["GiftCode"].Value + "";
                dateTimePickerCreate.Value = dgvUsedTrans.Rows[rindex].Cells["DateOfUse"].Value + "" == "" ? DateTime.Now : Convert.ToDateTime(dgvUsedTrans.Rows[rindex].Cells["DateOfUse"].Value + "");

                if (dgvUsedTrans.Rows[rindex].Cells["BranchId"].Value + "" != "") cboBranch.setBranchValue(dgvUsedTrans.Rows[rindex].Cells["BranchId"].Value + "");

                comboBoxCommission1.SelectedValue = EN_Helper = dgvUsedTrans.Rows[rindex].Cells["EN_Helper"].Value + "";
                ...
                txtRefMO.Text = dgvUsedTrans.Rows[rindex].Cells["RefMO"].Value + "";
                if (LastUsed.Date > dateTimePickerCreate.Value)//ปิดแก้ไข ถ้าเก่ากว่า
                {
                    //txtRefMO.ReadOnly = true;
                    txtUseNew.ReadOnly = true;
                    dateTimePickerCreate.Enabled = false;
                ...
                {
                    //txtRefMO.ReadOnly = false;
                    txtUseNew.ReadOnly = false;
                    dateTimePickerCreate.Enabled = true;
        ...
        private void btnToJob_Click(object sender, EventArgs e)
        {
            try
            {
              ...
              //  if (intStatus <= 0) return;
                ...
                DataTable dtSurgeryFee=new DataTable();
                if (IsSubItem == false)
                {
                    dsSurgeryFee=new Business.MedicalOrder().SelectMedicalOrderById(VN,Sono);
                    dtSurgeryFee = dsSurgeryFee.Tables[1];
                }
                else
                {
                    dsSurgeryFee = new Business.MedicalOrder().SelectMedicalOrderByIdSubitem(VN, Sono);
                    dtSurgeryFee = dsSurgeryFee.Tables[1];
                }
            ...
            {
                MessageBox.Show("PHARMACY OR Consult", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
                 ...
                 Statics.frmSurgicalFeeMain.UseTransID = useTransId;
        ...
        private void PopMedicalUsed_Shown(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    BindDataUsed(-1);
                    panel1Button.Visible = false;
                    dgvUsedTrans.Columns["BtnDelete"].Visible = false;
                }
            }
            catch (Exception  ex)
            {
        ...
        private void picBalManual_Click(object sender, EventArgs e)
        {
            //manulaBal = true;
            if (textboxFormatDoubleUseBal.Visible)
            {
                textboxFormatDoubleUseBal.Visible = false;
                labelUseBal.Visible = false;
            }
            else 
            {
                textboxFormatDoubleUseBal.Visible = true;
                labelUseBal.Visible = true;
            }
        }
        ...
        private void btnGiftcode_Click(object sender, EventArgs e)
        {
            try
            {
                   FrmFreeGiftVoucher frm = new FrmFreeGiftVoucher();
                    frm.GiftCode = txtGiftCode.Text;// dataGridViewSelectList.Rows[currentRowIndex].Cells["GiftNumber"].Value + "";
                    frm.ShowDialog();

                        txtGiftCode.Text=frm.GiftCode;
