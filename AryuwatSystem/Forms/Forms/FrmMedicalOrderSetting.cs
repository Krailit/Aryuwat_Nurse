...
namespace DermasterSystem.Forms
{
    public partial class FrmMedicalOrderSetting : DockContent, IForm
    {
            ...
            tabPromotion = 8,
             ...
             string MoSubType = "";
              ...
              private void showRef()
              {
                  if (this.RefVN+"" != "")
                  {
                      this.txtMoRef.Visible = true;
                      this.txtBalanceRef.Visible = true;
                      this.labelref1.Visible = true;
                      this.lblBalanceRef.Visible = true;
                      this.lblRefVN.Visible = true;
                      this.txtMoRef.Text = this.RefVN;
                      this.txtBalanceRef.Text = (this.PriceRef == "") ? "0" : Convert.ToDecimal(this.PriceRef).ToString("###,###,###.##");
        ...
        //        else
        //        {
        //             idMax = DermasterSystem.Data.UtilityBackEnd.GenMaxSeqnoValues("MO-");
        ...
        private void BindData()
        {
            ...
            this.RefVN = dtMed.Rows[0]["VNClose"]+"";
                        ...
                        if ((dr["MS_Code"] + "").Contains("PRO_CREDIT"))
                        {
                            ...
                            foreach (DataGridViewRow item in dgvPromotionList.Rows)
                            {
                                if ((dr["MS_Code"] + "").Contains(item.Cells["PRO_Code"].Value.ToString()))
                                {
                                    ...
                                    ProCredit = item.Cells["ProPriceCredit"].Value + "" == "" ? 0 : decimal.Parse(item.Cells["ProPriceCredit"].Value + "");
                                    //lbPromotion.Text = string.Format("Promotion:{0}:{1}", PRO_Code, item.Cells["PRO_Name"].Value + "");
                                    lbProCredit.Text = string.Format("ยอดเงินคงเหลือ/Credit {0} บาท/Bth.", ProCredit.ToString("###,###,###.##"));
        ...
        private void SetColumnDgvFile()
        {
            ...
            Utility.SetPropertyDgv(dgvFile);
        ...
        {
            Utility.SetPropertyDgv(dgvAestheticList);
            DataGridViewCheckBoxColumn column = new DataGridViewCheckBoxColumn();
            {
                column.AutoSizeMode =
                    DataGridViewAutoSizeColumnMode.DisplayedCells;
        ...
        {
            Utility.SetPropertyDgv(dgvTreatmentList);
            DataGridViewCheckBoxColumn column = new DataGridViewCheckBoxColumn();
            {
                column.AutoSizeMode =
                    DataGridViewAutoSizeColumnMode.DisplayedCells;
        ...
        {
            Utility.SetPropertyDgv(dgvSurgeryList);
            DataGridViewCheckBoxColumn column = new DataGridViewCheckBoxColumn();
            {
                column.AutoSizeMode =
                    DataGridViewAutoSizeColumnMode.DisplayedCells;
        ...
        {
            Utility.SetPropertyDgv(dgvWellness_AntiagingList);
            DataGridViewCheckBoxColumn column = new DataGridViewCheckBoxColumn();
            {
                column.AutoSizeMode =
                    DataGridViewAutoSizeColumnMode.DisplayedCells;
        ...
        {
            Utility.SetPropertyDgv(dgvPharmacyList);
            DataGridViewCheckBoxColumn column = new DataGridViewCheckBoxColumn();
            {
                column.AutoSizeMode =
                    DataGridViewAutoSizeColumnMode.DisplayedCells;
        ...
        {
            Utility.SetPropertyDgv(dgvHairList);
            DataGridViewCheckBoxColumn column = new DataGridViewCheckBoxColumn();
            {
                column.AutoSizeMode =
                    DataGridViewAutoSizeColumnMode.DisplayedCells;
            ...
            if (MoSubType == "PRO")//โปรธรรมดา
            {
                ...
                foreach (DataGridViewRow item in dv.Rows)
                {
                    if ((bool)item.Cells[0].Value == true)
                    {
                        ...
                        lbProCredit.Text = string.Format("ยอดเงินคงเหลือ/Credit {0} บาท/Bth.", ProCredit.ToString("###,###,###.##"));
                        lbProCredit.Visible = item.Cells["PRO_Type"].Value + "" == "CREDIT";
                        PRO = item.Cells["PRO_Type"].Value + "" == "CREDIT" ? "Y" : "";
                    }
                }
    ...
    private void buttonDeleteUp_BtnClick()
    {
        ...
        private void btnBrowse_Click(object sender, EventArgs e)
        {
            ...
            {
                if (Utility.PopMsg(Utility.EnuMsgType.MsgTypeConfirmYesNo, "หากคุณเปลี่ยนแปลง \"ชื่อลูกค้า \" รายการที่เลือกจะถูกยกเลิก \n\rคุณต้องการเปลี่ยนใช่หรือไม่?") == DialogResult.Yes)
                {
        ...
        private void dataGridViewSelectList_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                Utility.PopMsg(Utility.EnuMsgType.MsgTypeError, "เกิดข้อผิดพลาดในการแสดงข้อมูล เนื่องจาก " + ex.Message);
            }
        }
        ...
        private void SumPriceMedicalOrder()
        {
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridViewSelectList.Rows)
                {
                    if (row.Visible) SalePriceNew += row.Cells["PriceTotal"].Value + "" == "" ? 0 : decimal.Parse(row.Cells["PriceTotal"].Value + "");
                    
                    //if (FormType == Utility.AccessType.Update)
                    //{
                    //    if (row.Cells["ChkPRO"].Value + "" != "True") SalePriceNew += row.Cells["PriceTotal"].Value + "" == "" ? 0 : decimal.Parse(row.Cells["PriceTotal"].Value + "");
                    //    else
                    //    {
                ...
                //===============คำนวน Pro credit=========
                //SalePriceNew = ProCredit - SalePriceNew;
                if (SalePriceNew > ProCredit && ProCredit!=0) OverProCredit = true;
                ...
                if (OverProCredit)
                {
                    ...
                    Utility.PopMsg(Utility.EnuMsgType.MsgTypeConfirmYesNo, Statics.OverProCredit);
                    SumPriceMedicalOrder();
                }

                lbProCredit.Text = string.Format("ยอดเงินคงเหลือ/Credit {0} บาท/Bth.", (ProCredit-SalePriceNew) == 0 ? "0" : (ProCredit-SalePriceNew).ToString("###,###,###.##"));
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (Utility.PopMsg(Utility.EnuMsgType.MsgTypeConfirm, "ยืนยันการบันทึกข้อมูล") == DialogResult.OK)
            {
                if (string.IsNullOrEmpty(cn) )
                {
                    Utility.PopMsg(Utility.EnuMsgType.MsgTypeInformation, "กรุณาระบุ \"ชื่อลูกค้า \" ก่อนทำการบันทึกข้อมูล");
                    return;
                }
                try
                {
                    ...
                    info.CreateBy = Statics.StrEmployeeID;
                    info.Remark = "";
        ...
        private void btnAddFile_BtnClick()
        {
            ...
            if (txtFilePath.Text != "")
            {
                if (txtFileName.Text == "")
                {
                    Utility.PopMsg(Utility.EnuMsgType.MsgTypeInformation, "กรุณาระบุชื่อไฟล์");
                    return;
                }
                object[] myItems = {
                                       txtFilePath.Text,
        ...
        private void dgvFile_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.ColumnIndex == 3)
                {
                    if (Utility.PopMsg(Utility.EnuMsgType.MsgTypeConfirm, "ยืนยันการลบไฟล์") == DialogResult.OK)
                    {
        ...
        private void MergItemLoad(DataGridViewRow item)
        {
            try
            {
                ...
                rowsToDelete = new List<DataGridViewRow>();
                        ...
                        {
                            Utility.PopMsg(Utility.EnuMsgType.MsgTypeInformation, "ไม่สามารถรวมรายการได้ เนื่องจาก \"รายการไม่ใช่หมวดเดียวกัน\"");
                            return;
                        }
                        else
                        {
        ...
        private void radioButtonMO_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (radioButtonMO.Checked)
                {
                    //dataGridViewSelectList.Rows.Clear();
                    moso = radioButtonMO.Checked ? "MO-" : "SO-";
                    if (FormType != Utility.AccessType.Update || SO != "")
                    {
                        //this.idMax = UtilityBackEnd.GenMaxSeqnoValues(moso);
                        //this.txtMO.Text = this.MO = this.idMax.Replace("VNM", moso);
