    ...
    public partial class FrmPersonnelSetting : DockContent, IForm
    {
       ...
       private void FrmPersonnelSetting_Load_1(object sender, EventArgs e)
                ...
                if (pn != null && !string.IsNullOrEmpty(pn.EN)) //open
                {
                    ...
                    if (dsPersonnel != null && dsPersonnel.Tables.Count > 0)
                    {
                       ...
                       // if (dicPrefix.ContainsValue(dt.Rows[0]["PrefixCode"]+"".Trim()))
                        ...
                        //Bind IMage
                        if (!string.IsNullOrEmpty(Convert.ToString(dt.Rows[0]["ImageFilename"])))
                        {
                            _imagetPath = Properties.Settings.Default.ImagePathServer + @"\Personnels\" +
                                          Convert.ToString(dt.Rows[0]["ImageFilename"]);
                            picPersonnelImage.SizeMode = PictureBoxSizeMode.Zoom;
                            var callback = new Image.GetThumbnailImageAbort(ThumbnailCallback);
                            picPersonnelImage.Image = Image.FromFile(Properties.Settings.Default.ImagePathServer +
                                                                     @"\Personnels\" +
                                                                     Convert.ToString(dt.Rows[0]["ImageFilename"])).
                                GetThumbnailImage(
            ...
            try
            {
               ...
               : txtSubDistrict.Text.Trim();
                ...
                if (txtEmail.Text.Trim() != "")
                {
                    ...
                    {
                        Utility.PopMsg(Utility.EnuMsgType.MsgTypeError,"กรุณาระบุ \" รูปแบบ Email  \" ให้ถูกต้อง ก่อนทำการบันทึกข้อมูล ");
                        txtEmail.Focus();
                        return ;
                    }
                }
                ...
                {
                    MessageBox.Show("โปรดระบุ จังหวัด");
                    return;
                }
                if (string.IsNullOrEmpty(pn.District))
                {
                    MessageBox.Show("โปรดระบุ อำเภอ");
                    return;
                }
                if (string.IsNullOrEmpty(pn.Sub_district))
                {
                    MessageBox.Show("โปรดระบุ ตำบล");
                    return;
                }
                if (string.IsNullOrEmpty(pn.PostCode))
                {
                    MessageBox.Show("โปรดระบุ รหัสไปรษณีย์");
                    return;
                }
                if (string.IsNullOrEmpty(pn.IdCard) && string.IsNullOrEmpty(pn.PassportId))
                {
                        MessageBox.Show("โปรดระบุ เลขบัตรประชาชน หรือ Passport ID");
                        return;
                }
                ...
                {
                    MessageBox.Show("โปรดระบุ ชื่อผู้ใช้งานระบบ");
                    return;
                }
                if (string.IsNullOrEmpty(pn.Passwords))
                {
                    MessageBox.Show("โปรดระบุ รหัสผ่าน/หรือยืนยันรหัสผ่านไม่ถูกต้อง");
                    return;
                }
                if (string.IsNullOrEmpty(pn.UserGroup))
                {
                    MessageBox.Show("โปรดระบุ ระดับผู้ใช้งาน");
                    return;
                }
                ...
                switch (FormType)
                {
                    ...
                    case Utility.AccessType.Insert:
                        ...
                        if (intStatus == 1)
                        {
                            Statics.frmPersonnelList.BindDataCustomer(1);
        ...
        private void btnCamera_BtnClick()
        {
            ...
            {
                picPersonnelImage.ImageLocation =
                    _imagetPath = Application.StartupPath + @"\PersonnelImage\" + webCapture.EN + ".jpg";
                _Changimage = true;
            }
        }
        ...
        {
            picBrown.Image = global::DermasterSystem.Properties.Resources.Find;
        }

        private void picBrown_MouseHover(object sender, EventArgs e)
        {
            picBrown.Image = global::DermasterSystem.Properties.Resources.FindOver;
        }
        ...
        private void textPass2_Leave(object sender, EventArgs e)
        {
            if (!txtPass1.Text.Equals(txtPass2.Text))
            {
                txtPass2.Focus();
                txtPass2.SelectAll();
                MessageBox.Show("ยืนยันรหัสผ่านไม่ถูกต้อง");
                return;
            }
        ...
        private void txtUsername_Leave(object sender, EventArgs e)
        {
            try
            {
                if(FormType == Utility.AccessType.Insert || FormType == Utility.AccessType.Update)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        MessageBox.Show("มีผู้ใช้งานอื่นใช้ ชื่อนี้แล้ว");
                        txtUsername.Focus();
                        txtUsername.Text = "";
