...
{
    public partial class popReplySupplies : DockContent
    {
        ...
        private void SaveREQStock(string App)//ไม่ reset ก่อน
        {
            try
            {
                ...
                foreach (DataGridViewRow item in dataGridViewSelectList.Rows)
                {
                    ...
                    aa.QueryType = "UPDATE_REPLY_SP_STOCK";//INSER_REQ_STOCK
        ...
        { 
            try
            {
                //List<string> LsMS_Codex = new List<string>();
                //DataTable dtP = dtReq.Clone();
                //foreach (DataRow item in dtReq.Rows)
                //{
                //    //if (LsMS_Codex.Contains(item["MS_Code"] + "") || item["MS_Code"] + "" == "") continue;
                //    item["Quantity"]=Quantity
                //    dtP.ImportRow(item);
                //}
                var newDt = dtReq.AsEnumerable()
              ...
              .Select(g =>
              {
                ...
                FrmPreviewRpt obj = new FrmPreviewRpt();
                obj.ForDate = string.Format(" วันที่ {0} - {1}", String.Format("{0:dd/MM/yyyy}", DateTime.Parse(dateTimePickerSt.Value + "")), String.Format("{0:dd/MM/yyyy}", DateTime.Parse(dateTimePickerEnd.Value+"")));
                obj.FormName = "RptREQREPInventory";
