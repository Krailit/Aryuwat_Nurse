...
{
    public partial class FrmCommonReportSaleResultByCN : DockContent
    {
        bool bind = true;
        ...
        DataSet dsData;
        public FrmCommonReportSaleResultByCN()
        {
            InitializeComponent();
        }
        ...
        public void BindReportResult(int pIntseq)
        {
            try
            {
                ...
                //        info.QueryType = "ReportSaleResult_Year_ByCN";
                ...
                //    else if (cboSurgicalFeeTyp.Text == "PHAMACY")
              ...
              if (comboBoxMonth.SelectedIndex==12)
                ...
                else
                {
                    ...
                    info.QueryType = "ReportSaleResult_Month_ByCN";
                }
                ...
                dgvData.Rows[dgvData.RowCount - 1].DefaultCellStyle.Font = new Font(DataGridView.DefaultFont.Name, varFontSize, FontStyle.Bold);
                dgvData.Rows[dgvData.RowCount - 1].DefaultCellStyle.BackColor = Color.PowderBlue;
                
                dgvData.Rows[dgvData.RowCount - 1].Height = dgvData.Rows[dgvData.RowCount - 1].Height + 2;
                //dgvData.Columns[dgvData.Columns.Count - 1].DefaultCellStyle.Font = new Font("Tahoma", varFontSize, FontStyle.Bold);


                dgvData.Columns[dgvData.Columns.Count - 1].DefaultCellStyle.BackColor = Color.PowderBlue;
                //dgvData.Columns[dgvData.Columns.Count - 1].DefaultCellStyle.Font = Color.ForestGreen;
                for (int i = 1; i < dgvData.Columns.Count; i++)
                {
                    //if (dgvData.Columns[i].Name.ToLower() != "ms_name" && dgvData.Columns[i].Name.ToLower() != "ms_code")
                    dgvData.Columns[i].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                }
                BarChart(dsData.Tables[0]);
            }
            catch (Exception ex)
            {
        ...
        public void BarChart(DataTable dt)
        {
            this.chart1.Series.Clear();
            //DataTable 
            //    dt = PivotTable.GenerateTransposedTable(dt);
                ...
                if (dt!= null)
                {
                    ...
                    //    if (chart1.Series.FindByName(dt.Columns[colIndex].ColumnName) == null)
                    //    {
                    //        series = dt.Columns[colIndex].ColumnName;
                    ...
                    //    {
                    //        double dataPoint = 0;
                    //        double.TryParse(dt.Rows[row][dt.Columns[colIndex].ColumnName].ToString(), out dataPoint);
                    //        DataPoint objDataPoint = new DataPoint() { AxisLabel = "series", YValues = new double[] { dataPoint } };
                    //        chart1.Series[series].Points.AddXY(dt.Rows[row][0].ToString(), dataPoint);
                    //    }
                    //}
                    for (int colIndex = 1; colIndex < dt.Columns.Count - 1; colIndex++)
                    {
            ...
            //    string seriesName = dt.Rows[row]["CustomerType"].ToString();
            //    chart1.Series.Add(seriesName);
            //    chart1.Series[seriesName].ChartType = SeriesChartType.Column;
            //    chart1.Series[seriesName].BorderWidth = 4;

            //    for (int colIndex = 1; colIndex < dt.Columns.Count-1; colIndex++)
            //    {
            //        // For each column (column 1 and onward) add the value as a point
            //        string columnName = dt.Columns[colIndex].ColumnName;
            //        double YVal = dt.Rows[row][columnName] + "" == "" ? 0 : Convert.ToDouble(dt.Rows[row][columnName]);

            //        chart1.Series[seriesName].Points.AddXY(YVal, columnName);
            //    }
            //}
        }
        ...
        private void FrmCommonReportSaleResultByCN_Load(object sender, EventArgs e)
        {
        ...
        private void BindCNType()
        {
            try
            {
                DataSet dsCN = new Business.Customer().SelectDistinctCN();

                DataTable dt = dsCN.Tables[0];
                cboSurgicalFeeTyp.Items.Clear();

                foreach (DataRow item in dt.Rows)
                {
                    cboSurgicalFeeTyp.Items.Add(item["CustomerType"]);
                }
                cboSurgicalFeeTyp.Items.Add("All");
                cboSurgicalFeeTyp.SelectedIndex = dt.Rows.Count;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        private void FrmCommonReportSaleResultByCN_FormClosing(object sender, FormClosingEventArgs e)
        {
            Statics.frmCommonReportSaleResultByCN = null;
        }
        ...
        private void dgvData_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            //    dgvData.Columns[e.ColumnIndex].DefaultCellStyle.Font = new Font("Tahoma", varFontSize, FontStyle.Bold);
            //    dgvData.Rows[dgvData.RowCount - 1].DefaultCellStyle.Font = new Font(DataGridView.DefaultFont.Name, varFontSize, FontStyle.Bold);
