...
{
    public partial class FrmReportMarketing : DockContent
    {
        bool bind = true;
        ...
        private void GetReportName()
        {
            try
            {
                Entity.Report info = new Entity.Report() { PageNumber = 1 };
                    info.QueryType = "GetReportName";

                    info.StartDate = DateTime.Now.ToString();
                    info.EndDate = DateTime.Now.ToString();
                    DataSet ds = new Business.ReportMarketing().SelectReportMarketing(info);
        ...
        public void BindReport(int _pIntseq)
        {
            try
            {
                ...
                info.SalePrice = txtMoneyUp.TextLength > 3 ? Convert.ToDecimal(txtMoneyUp.Text) : 0;
                info.BirthMonth = (comboBoxMonth.SelectedIndex + 1).ToString();
                DataSet ds = new Business.ReportMarketing().SelectReportMarketing(info);
                dtAllTotal=ds.Tables[0];
                long lngTotalPage = 0;
                ...
                for (int i = 0; i < dgvData.Columns.Count; i++)
                {
                    if (dgvData.Columns[i].Name.ToLower().Contains("จำนวน") || dgvData.Columns[i].Name.ToLower().Contains("ราคา") || dgvData.Columns[i].Name.ToLower().Contains("เฉลี่ย") || dgvData.Columns[i].Name.ToLower().Contains("quan") || dgvData.Columns[i].Name.ToLower().Contains("ค่า")
                        || dgvData.Columns[i].Name.ToLower().Contains("เงิน") || dgvData.Columns[i].Name.ToLower().Contains("จำนวน") || dgvData.Columns[i].Name.ToLower().Contains("ยอด") || dgvData.Columns[i].Name.ToLower().Contains("ลด") || dgvData.Columns[i].Name.ToLower().Contains("price"))
                        dgvData.Columns[i].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                }
        ...
        private void FrmReportMarketing_Load(object sender, EventArgs e)
        {
        ...
        private void FrmReportMarketing_FormClosing(object sender, FormClosingEventArgs e)
        {
            Statics.frmReportMarketing = null;
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (QueryType == "RptAccBenefitAESTHETIC")
                {
                    obj.PrintType = string.Format(" AESTHETIC วันที่ {0} - {1}", txtStartdate.Text, txtEnddate.Text);
                }

                else if (QueryType == "RptAccBenefitTREATMENT")
                {
                    obj.PrintType = string.Format(" TREATMENT วันที่ {0} - {1}", txtStartdate.Text, txtEnddate.Text);
                }
                else if (QueryType == "RptAccBenefitANGTI-AGING")
                {
                    obj.PrintType = string.Format(" ANGTI-AGING วันที่ {0} - {1}", txtStartdate.Text, txtEnddate.Text);
                }
        ...
        {
            if (e.Value + "" == "0.00" || e.Value + ""=="") e.Value = "";
            if (dgvData.Columns[e.ColumnIndex].Name.Contains("ค่า") && e.Value!="")
        ...
        private void comboBoxReport_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                //}
             QueryType=  dicReporQueryType[comboBoxReport.SelectedValue.ToString()];
             if (QueryType == "RptMktBirthDateSell")
             {
                 labelMonth.Visible = true;
                 comboBoxMonth.Visible = true;
             }
        ...
        private void buttonPrint1_BtnClick()
        {
            try
            {
                ...
                else if (QueryType == "RptSendToCustomer")
                {
                    obj.PrintType = string.Format(" ข้อมูล วันที่ {0} - {1}", txtStartdate.Text, txtEnddate.Text);
                }
                else if (QueryType == "RptAccBenefitTREATMENT")
                {
                    obj.PrintType = string.Format(" TREATMENT วันที่ {0} - {1}", txtStartdate.Text, txtEnddate.Text);
                ...
                obj.dt = dtAllTotal.DefaultView.ToTable(); 
                obj.MaximizeBox = true;
                obj.TopMost = true;
                obj.Show();
            }
